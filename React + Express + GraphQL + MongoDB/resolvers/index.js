...
const categorytitle = async(category, language) => {
  return title = await Title.findOne({category: category, language: language}).exec();
}
...
const CatTitle = async(catID, slanguage) => {
  const res = await Title.findOne({category: catID, language: slanguage}).exec();
  return res;
}
...
module.exports = {
 ...
 articlesPhone: async () => {
  const language = await Language.findOne({shorthand: "en"}).exec();
  return Article.find().then(articles => {
    return articles.map(article => {
      return {...article._doc,
        title: title.bind(this, article._doc._id, language._id)
      };
    });
  });
},
...
category: async({articleId}) => {
  const article =  await Article.findOne({_id: articleId}).exec();
  const category = await Category.findOne({_id: article.category}).exec();
  return category;
},
categories: async({language}) => {
  const clanguage = await Language.findOne({shorthand: language}).exec();
  return Category.find().then(
    (categories) => {
      return categories.map((category) => {
        return {...category._doc, 
          titles: CatTitle.bind(this, category._doc._id, clanguage._id)
        }
      });
    }
  )
},
categorize: async({articleId, category}) => {
  const article = await Article.findOne({_id: articleId}).exec();
  const ccategory = await Category.findOne({_id: category}).exec();
...
uploadPicture: async({url, article, language}, req) => {
  if(!req.isAuthorized) {
    throw new Error("Articles have to be created by users.")
  }
  const carticle = await Article.findOne({_id: article}).exec();
  const clanguage = await Language.findOne({shorthand: language}).exec();
  ...
  const picture = await Picture({
    url: url,
    article: article,
    language: clanguage._id,
    uploader: req.userId,
    uploaded: udate
  });
  const uPicture = await picture.save();
  carticle.picture.push(uPicture);
  await carticle.save();
  const modification = await Modification({
    article: article,
    language: clanguage._id,
...
picture: async({articleId, language}) => {
  const clanguage = await Language.findOne({shorthand: language}).exec();
  const picture = await Picture.findOne({article: articleId, language: clanguage._id}).exec();
  return picture; 
},
...
people: async(args) => {
  const category = await Category.findOne({shorthand: "p"}).exec();
  const slanguage = await Language.findOne({shorthand: args.language}).exec();
  return Article.find({languages: slanguage._id, category: category._id}).then(
     (articles) => {
        return articles.map((article) => {
          return {...article._doc,
