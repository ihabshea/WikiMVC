...
const InfoBox = ({classes, editMode, authD, title, setTitle, fetchTitle,
  ...
  let nodeleteInput = false;
    ...
    const [noPicture, setNP] = useState(false);
...
const fetchFields = async() => {
   ...
   const fetchPicture = async() => {
    ...
    // console.log(lpreferredLanguage,articleId);
     ...
     query : `
       query{
         picture(articleId:"${articleId}", language:"${lpreferredLanguage}"){
           url
         }
        }
     `
   ...
   return res.json();
     ...
     console.log(resData);
     if(resData.data.picture){
       setPic(resData.data.picture.url);
     }else{
       setNP(true);
     }
   }).catch(err => {
     ...
     useEffectAsync(async () => {
      ...
      const uploadPicture = async () => {
        ...
        query : `
        mutation {
          uploadPicture(url: "${picture}", article: "${articleId}", language: "${lpreferredLanguage}"){
            _id
          }
        }`
        };
    ...
    return (
        ...
        <ImageUploader
                withIcon={true}
                withPreview={true}
                buttonText='Upload an image for this article'
                onChange={async (pictured, pictureURL) => {   await setPicture(pictureURL[0]); console.log(pictureURL[0],picture); uploadPicture(); }}
                singleImage={true}
